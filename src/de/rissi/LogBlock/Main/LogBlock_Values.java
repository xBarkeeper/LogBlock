...
public class LogBlock_Values
{
	...
	{ "CREATE TABLE IF NOT EXISTS `" + DATABASE_DBNAME + "`.`" + DATABASE_TABLE_BLOCK + "` (" + "`"
			+ COLUMNNAME_BLOCK_ID + "` INT NOT NULL AUTO_INCREMENT," + "`" + COLUMNNAME_BLOCK_TYP
			+ "` VARCHAR(100) NOT NULL," + "`" + COLUMNNAME_BLOCK_COORDS + "` VARCHAR(50) NOT NULL," + "`"
			+ COLUMNNAME_BLOCK_EDITTIME + "` DATETIME NOT NULL," + "`" + COLUMNNAME_BLOCK_ACTION
			+ "` VARCHAR(6) NOT NULL," + "`" + COLUMNNAME_PLAYER_UUID + "` VARCHAR(36) NOT NULL," + "PRIMARY KEY (`"
			+ COLUMNNAME_BLOCK_ID + "`))", };
	...
	public static boolean playerIsInEditMode(Player p)
	{
		...
		if (LogBlock_Values.inEditMode.get(p.getUniqueId()) == true)
		{
			return true;
		} else if (LogBlock_Values.inEditMode.get(p.getUniqueId()) == false)
		{
			return false;
		}else {
			return false;
